"""
==============================================================
  [APP_NAME] Registration System — Flask Backend
  -----------------------------------------------
  PLACEHOLDERS (replace before use):
    [APP_NAME]          → e.g. "EventHub", "TicketMaster Pro"
    [ITEM_LABEL]        → singular label, e.g. "Registration", "Ticket"
    [ITEMS_LABEL]       → plural label,   e.g. "Registrations", "Tickets"
...
app.config["SQLALCHEMY_DATABASE_URI"] = "sqlite:///[APP_NAME_LOWER].db"
...
# ──────────────────────────────────────────────
CATEGORY_OPTIONS = [
    "[CATEGORY_1]",   # e.g. "General Admission", "VIP", "Early Bird"
    "[CATEGORY_2]",
    "[CATEGORY_3]",
    "Other",
]
...
class User(db.Model):
    ...
    items      = db.relationship("[ITEM_LABEL]", backref="owner", lazy=True)
...
class [ITEM_LABEL](db.Model):
    ...
    """
    __tablename__ = "[ITEMS_LABEL_LOWER]"
    id          = db.Column(db.Integer, primary_key=True)
    name        = db.Column(db.String(200), nullable=False)   # [ITEM_LABEL] title / name
...
def dashboard():
    user = current_user()
    all_items   = [ITEM_LABEL].query.filter_by(user_id=user.id).all()
    active_cnt  = sum(1 for i in all_items if i.status == "Active")
    inactive_cnt = len(all_items) - active_cnt
    recent_items = [ITEM_LABEL].query.filter_by(user_id=user.id)\
                               .order_by([ITEM_LABEL].created_at.desc())\
                               .limit(10).all()
...
def list_items():
    ...
    query  = [ITEM_LABEL].query.filter_by(user_id=user.id)
    if search:
        query = query.filter([ITEM_LABEL].name.ilike(f"%{search}%"))
    items = query.order_by([ITEM_LABEL].created_at.desc()).paginate(page=page, per_page=10)
    return render_template("items.html", items=items, search=search,
                           item_label="[ITEM_LABEL]", items_label="[ITEMS_LABEL]")
...
def create_item():
    ...
    db.session.add(item)
    db.session.commit()
    flash("[ITEM_LABEL] created successfully.", "success")
...
def show_item(item_id):
    item = [ITEM_LABEL].query.get_or_404(item_id)
...
def update_item(item_id):
    ...
    db.session.commit()
    flash("[ITEM_LABEL] updated.", "success")
    return redirect(url_for("list_items"))
