"""
==============================================================
  Food Registration Registration System — Flask Backend
  -----------------------------------------------
  PLACEHOLDERS (replace before use):
    Food Registration          → e.g. "EventHub", "TicketMaster Pro"
    Order        → singular label, e.g. "Registration", "Ticket"
    Orders       → plural label,   e.g. "Registrations", "Tickets"
...
app.config["SQLALCHEMY_DATABASE_URI"] = "sqlite:///food_registration.db"
...
# ──────────────────────────────────────────────
CATEGORY_OPTIONS = [
    "Dine-in",   # e.g. "General Admission", "VIP", "Early Bird"
    "Takeaway",
    "Delivery",
    "Other",
]
...
class User(db.Model):
    ...
    items      = db.relationship("Order", backref="owner", lazy=True)
...
class Order(db.Model):
    ...
    id          = db.Column(db.Integer, primary_key=True)
    name        = db.Column(db.String(200), nullable=False)   # Order title / name
...
def dashboard():
    user = current_user()
    all_items   = Order.query.filter_by(user_id=user.id).all()
    active_cnt  = sum(1 for i in all_items if i.status == "Active")
    inactive_cnt = len(all_items) - active_cnt
    recent_items = Order.query.filter_by(user_id=user.id)\
...
def list_items():
    ...
    items = query.order_by(Order.created_at.desc()).paginate(page=page, per_page=10)
    return render_template("items.html", items=items, search=search,
                           item_label="Order", items_label="Orders")
