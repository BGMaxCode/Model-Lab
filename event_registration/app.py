"""
==============================================================
  Event Registration Registration System — Flask Backend
  -----------------------------------------------
  PLACEHOLDERS (replace before use):
    Event Registration          → e.g. "EventHub", "TicketMaster Pro"
    Registration        → singular label, e.g. "Registration", "Ticket"
    Registrations       → plural label,   e.g. "Registrations", "Tickets"
...
# ──────────────────────────────────────────────
CATEGORY_OPTIONS = [
    "General Admission",   # e.g. "General Admission", "VIP", "Early Bird"
    "VIP",
    "Early Bird",
    "Other",
]
...
class User(db.Model):
    ...
    items      = db.relationship("Registration", backref="owner", lazy=True)
...
class Registration(db.Model):
    ...
    id          = db.Column(db.Integer, primary_key=True)
    name        = db.Column(db.String(200), nullable=False)   # Registration title / name
...
def dashboard():
    user = current_user()
    all_items   = Registration.query.filter_by(user_id=user.id).all()
    active_cnt  = sum(1 for i in all_items if i.status == "Active")
    inactive_cnt = len(all_items) - active_cnt
    recent_items = Registration.query.filter_by(user_id=user.id)\
...
def list_items():
    ...
    query  = Registration.query.filter_by(user_id=user.id)
    if search:
        query = query.filter(Registration.name.ilike(f"%{search}%"))
    items = query.order_by(Registration.created_at.desc()).paginate(page=page, per_page=10)
    return render_template("items.html", items=items, search=search,
                           item_label="Registration", items_label="Registrations")
...
def show_item(item_id):
    item = Registration.query.get_or_404(item_id)
