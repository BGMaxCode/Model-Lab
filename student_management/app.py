"""
==============================================================
  Student Management Registration System — Flask Backend
  -----------------------------------------------
  PLACEHOLDERS (replace before use):
    Student Management          → e.g. "EventHub", "TicketMaster Pro"
    Student        → singular label, e.g. "Registration", "Ticket"
    Students       → plural label,   e.g. "Registrations", "Tickets"
...
# ──────────────────────────────────────────────
CATEGORY_OPTIONS = [
    "Undergraduate",   # e.g. "General Admission", "VIP", "Early Bird"
    "Graduate",
    "PhD",
    "Other",
]
...
class User(db.Model):
    ...
    items      = db.relationship("Student", backref="owner", lazy=True)
...
class Student(db.Model):
    ...
    id          = db.Column(db.Integer, primary_key=True)
    name        = db.Column(db.String(200), nullable=False)   # Student title / name
...
def dashboard():
    user = current_user()
    all_items   = Student.query.filter_by(user_id=user.id).all()
    active_cnt  = sum(1 for i in all_items if i.status == "Active")
    inactive_cnt = len(all_items) - active_cnt
    recent_items = Student.query.filter_by(user_id=user.id)\
...
def list_items():
    ...
    query  = Student.query.filter_by(user_id=user.id)
    if search:
        query = query.filter(Student.name.ilike(f"%{search}%"))
    items = query.order_by(Student.created_at.desc()).paginate(page=page, per_page=10)
    return render_template("items.html", items=items, search=search,
                           item_label="Student", items_label="Students")
